...
def index(request):
    if request.method == "GET":
        ...
        modified_files = process_files(files)

        aula = render(request,'wordconvert.html', {'modified_files': modified_files})
        
        # Adicione o arquivo processado à lista de arquivos modificados
        return aula
...
def process_files(files):
    ...
    for file in files:
        ...
        exit_bt = soup.new_tag("a", **{'class': "btn btn-success close", 'id': "sair-aula-btn", 'style': "display:none;", 'data-action':"hide", 'aria-label':"Completar Aula" , 'onclick':"location.reload()"})
        ...
        modified_files.append(str(content_combined + ".html"))
